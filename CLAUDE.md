...
- `conformer_asr/encoders/` — full speech encoders. Default `conformer` → `MelConformerEncoder` (Wav2Vec2ConformerEncoder + a pluggable downsampler + SpecAugment on post-stem hidden states). Must expose `.config.hidden_size`, return a `BaseModelOutput` from `forward`, and provide `_get_feature_vector_attention_mask` for the decoder-side mask — `ConformerAEDWithCTC` and `scripts/evaluate.py` both call it by name.
